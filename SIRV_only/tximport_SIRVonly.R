...
files = list.dirs(path = '../Results/20180412/SIRV_only', full.names = F)
files = grep(pattern = '^[A-Z][0-9]{2}$', x = files, value = T)
files = file.path('../Results/20180412/SIRV_only', files , 'abundance.h5')

### Create exon to gene reference table from first sample
tpm = read.table(file = '../Results/20180412/SIRV_only/F58/abundance.tsv', header = T, row.names = 1)
IDtable = data.frame(transcriptID = row.names(tpm))
# Create gene names as substrings: SIRV and one numeric & any non t character after the start (WIP)
